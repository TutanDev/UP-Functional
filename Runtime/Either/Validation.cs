...
namespace TD.Functional
{
	...
	{
		internal IEnumerable<Error> _Errors { get; }
		...
		private Validation(IEnumerable<Error> errors) => (IsValid, _Errors, _Value) = (false, errors, default);

		internal Validation(T t) => (IsValid, _Errors, _Value) = (true, Enumerable.Empty<Error>(), t);

		public static implicit operator Validation<T>(Error error) => new Validation<T>(new[] { error });
		public static implicit operator Validation<T>(Validation.Invalid left) => new Validation<T>(left._Errors);
		...
		public R Match<R>(Func<IEnumerable<Error>, R> Invalid, Func<T, R> Valid)
			=> IsValid ? Valid(this._Value!) : Invalid(this._Errors);
		...
		public override string ToString() => Match(errs => $"Invalid([{string.Join(", ", errs)}])", t => $"Valid({t})");

		public override bool Equals(object obj)
			=> obj is Validation<T> other
				&& this.IsValid == other.IsValid
				&& (IsValid && this._Value!.Equals(other._Value)
				|| this.ToString() == other.ToString());

		public override int GetHashCode() => Match(Invalid: errs => errs.GetHashCode(), Valid: t => t!.GetHashCode());
	}
	...
	public static class Validation
	{
		...
		{
			internal IEnumerable<Error> _Errors;
			public Invalid(IEnumerable<Error> errors) { _Errors = errors; }
		}
